...
func parseSubset(_ input: String) -> Subset {
    ...
    let reds = colors.filter { String($0).split(separator: " ").last == "red" }.first?.split(separator: " ").first ?? "0"
    let greens = colors.filter { String($0).split(separator: " ").last == "green" }.first?.split(separator: " ").first ?? "0"
    let blues = colors.filter { String($0).split(separator: " ").last == "blue" }.first?.split(separator: " ").first ?? "0"
...
public func smallestPossibleBag(_ subsets: [Subset]) -> Subset {
    let (reds, greens, blues) = subsets.reduce((0, 0, 0), { (max($0.0, $1.reds), max($0.1, $1.greens), max($0.2, $1.blues)) })
