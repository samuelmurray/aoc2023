...
class ParseTests: XCTestCase {
    ...
    func testSmallInput() {
        let result = computeSum(["Game 2: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green"], Subset(reds: 12, greens: 13, blues: 14))
        XCTAssertEqual(2, result)
    }

    func testSmallInputFalse() {
        let result = computeSum(["Game 2: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green"], Subset(reds: 5, greens: 5, blues: 5))
        XCTAssertEqual(0, result)
    }

    func testErroneousInput() {
        let (_, subsets) = parseGame("Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red")
        print(subsets)
    ...
    func testGivenInput1() {
        let input = readFile("Resources/02/test-input1.txt")
        XCTAssertEqual(8, computeSum(input, Subset(reds: 12, greens: 13, blues: 14)))
    }

    func testInput1() {
        let input = readFile("Resources/02/input1.txt")
